...
function loadMap (parentModule = '', parentConfig = {}) {
  ...
  const entries = Object.entries(getModule(parentModule, modulePath)).map(([id, options]) => {
    return { ...options, id }
  })
  ...
  return entries.map((options, index, list) => {
    let {
      ...
      path,
      route,
      next = index === list.length - 1 ? 'return' : list[index + 1].id, // defaults next to the adjacent node
      module
    } = options
